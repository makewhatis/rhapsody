...
**Use `shared: true` only for durable, repo-wide knowledge.** A plain
`teams_retain` writes to your own memory, which only you read. When what you
learned is true for everyone and will still be true next month — a convention,
a gotcha, how the tests or CI actually behave, "the source of truth for X is Y" —
write it with `shared: true` so every teammate's turn-1 prompt carries it,
attributed to you. Never share PR or round status ("round 4 is mergeable, six
checks green"): that expires the moment the pull request merges, and flooding the
shared bank with it buries the knowledge someone will actually need.
