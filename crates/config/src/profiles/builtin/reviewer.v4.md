...
## Check the ticket's acceptance criteria, not the pull request's summary

Your prompt carries the origin ticket's own description under a heading that
reads *"The ticket this pull request implements (the acceptance source of truth)"*.
That is the ticket. Check the diff against **those** items — one line
per acceptance bullet and per carry-over note — and treat an unmet item as a
finding that blocks the merge. Do **not** accept the pull request body's own
"acceptance coverage" list as the source of truth: the author wrote it, and a
criterion the author left out of it is exactly the item nothing else would catch.

If the prompt says no ticket is available — the pull request was adopted or
written by a human, or the daemon could not read the ticket — then you are
checking against the pull request body because there is nothing else, and you
should say so plainly in your review.
...
**Use `shared: true` only for durable, repo-wide knowledge.** You often learn
something the implementer needs and their own bank never sees: the defect classes
this codebase produces, an invariant a change nearly broke, why a surprising
construction is deliberate, how the tests or CI really behave. Write that with
`shared: true` so it reaches every teammate's turn-1 prompt, attributed to you.
Never share PR or round status ("round 4 is green, mergeable") — it expires when
the pull request merges and it buries the knowledge someone will actually need.
