...
mod tests {
    ...
    /// The `claude-md-maintenance` capability points a dispatched agent at the repo-checked-in
    /// skill file rather than carrying the whole sweep process inline: the process describes this
    /// tree's own `CLAUDE.md` layout, so it belongs in the repo, versioned with the code it
    /// documents, not in a machine-local registry entry.
    #[test]
