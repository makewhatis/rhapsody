...
//! step 11 (`rhapsody_review_bound`, STUDIO-956) and step 12 (`rhapsody_review_done`, STUDIO-1007)
//! have no Go counterpart: they are the ticketless
...
///
/// Go v0.4.0 froze at 6. Steps 7, 8, 9, 10, 11 and 12 are Rhapsody-only (the ticketless review
/// watch set, then its `author` column, then the summons watermark, then per-run provenance, then
/// the per-pull-request review bound, then the durable terminal-move ledger) and are the one
...
"#,
    // v11 -> v12: the DURABLE terminal-move ledger — one row per ticket whose merged pull request
    // owes a move to its terminal state (STUDIO-1007). Rhapsody-only, so the `rhapsody_` prefix
    // gates it out of the Go-recaptured schema golden by name exactly as steps 7-11 are.
    //
