...
1. **Move the ticket to review — this is what ends your run.** Set {{ issue.identifier }} to the **"In Review"** state with `mcp__claude_ai_Linear__save_issue` (state: `In Review`). Do this with confidence: it is a non-active state, so once it lands the daemon stops giving you turns and records the run complete. If the write is denied, say so plainly in your final message (a human will move it) — do NOT retry it, and do NOT keep working.
...
If the work is ambiguous, hits a spec/plan contradiction, or needs something only a human can resolve (the reference path missing, a `gh api` 403 on branch protection, credentials), stop. Commit what is safely committable, comment the exact blocker on the ticket (once), move the ticket to **In Review** with `mcp__claude_ai_Linear__save_issue` so it parks cleanly for a human, and write a final message naming the blocker, your options, and your recommendation, ending with `HANDOFF: in-review`. Never guess on irreversible choices, and never loop retrying a failing or denied operation.
