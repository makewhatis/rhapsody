...
argv = sys.argv[4:]
t0 = time.time()
def el(): return f"{time.time()-t0:7.3f}s"
print(f"[{label}] argv={argv}")
print(f"[{label}] cwd={cwd} wait={wait_s}s at={time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}")
...
print(f"[{label}] {el()} pid={p.pid} pgid={pgid} daemon_pgid={os.getpgid(0)}")
time.sleep(wait_s)
...
print(f"[{label}] {el()} SURVIVORS AFTER kill(-{pgid}, SIGKILL): {len(alive)}")
